...
# `_run_git`, `_build_stamp` and `producing_version` moved to
# `oneground/provenance.py` and are re-exported here, so every importer of
# `receipts.producing_version` is unaffected and there is still one of each.
#
# They moved because the lab's server has to be able to say which build it is
# serving, and this module imports torch -- which the lab's guard refuses
# every served module. The choice was a second implementation of *what commit
# is this* or one more leaf; see that module's header for what it cost.
from ..provenance import (  # noqa: E402,F401
    BUILD_STAMP_NAME, _build_stamp, _run_git, producing_version)
