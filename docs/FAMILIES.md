...
**The seven warnings that were here are now `docs/PRACTICE.md` §2, "Checks
that do not check", with their numbers unchanged** — they are a general
taxonomy of checks that report something other than what they appear to, only
the first two are about this suite, and they belong with the other rules this
project has paid for. Read them before adding a check: warnings 3, 5 and 6 are
the ones that decide whether the check you add is evidence of anything, and
warning 7 is the one that will break a test you did not write.
